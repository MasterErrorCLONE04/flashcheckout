...
const FLASHY_SYSTEM_PROMPT = `Eres Flashy, el agente inteligente y consultor de ventas oficial de Flashcheckouts. Tu rol es atender a los visitantes de la landing page pública.
Tu personalidad es entusiasta, persuasiva, veloz, muy amable y profesional.
Tu meta es explicar qué es Flashcheckouts, resolver dudas de clientes potenciales (precios, integraciones de pago, funcionamiento técnico) y convencerlos de registrarse gratis en la plataforma.
...
Información clave de Flashcheckouts para responder:
...
const getSpanishFallbackResponse = (input: string): string => {
  ...
  if (text.includes('hola') || text.includes('buen') || text.includes('saludo') || text.includes('hi') || text.includes('hello')) {
    return "👋 ¡Hola! Soy **Flashy**, tu asistente inteligente de Flashcheckouts. ¿En qué puedo ayudarte hoy?\n\nPregúntame sobre:\n* ¿Qué es Flashcheckouts y cómo funciona?\n* Métodos de pago e integraciones (Stripe / MercadoPago).\n* Planes y precios (Plan Gratis vs Pro).\n* Cómo probarlo gratis hoy mismo.";
  }

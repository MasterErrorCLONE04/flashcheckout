...
type ReportBody = Record<string, never>
...
export async function POST(
  ...
  try {
    void (await req.json().catch(() => ({} as ReportBody)))
    ...
    if (updatedStore.verificationLevel === 0 && updatedStore.strikes >= 1) {
      ...
      try {
        ...
        await waClient.sendText(
          ...
          `TIENDA PAUSADA POR DISPUTA.\n\nTu tienda *${updatedStore.name}* ha sido pausada automaticamente tras recibir un reporte de fraude por parte de un comprador para el pedido *#${order.id.slice(-6).toUpperCase()}*.\n\nPara reactivar tu tienda y bot de ventas de inmediato, debes verificar tu identidad en el panel: ${appUrl}/verificaciones`
        )
      } catch (waErr) {
